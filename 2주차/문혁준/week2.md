...
- 분산 시스템의 비즈니스 이용 개척
    ...
    - 비교적 작고, 중요한 데이터 처리 → 전통적 데이터 웨어하우스
    - 전통적 데이터 웨어하우스와 Hadoop의 공존
...
### 1. 데이터 마트를 비정규화 테이블로 구성하는 것이 가장 단순하고 효율적인 방식이라고 했는데, 그 이유에 대해 함께 논의

데이터 엔지니어의 역할이 무엇일까 생각해보며 책을 읽어 보았습니다.

비정규화 테이블이란?

**분석의 편의를 위해 정규화된 여러 테이블의 정보를 하나로 합치고, 중복을 허용한 테이블**

열 지향 스토리지란?

데이터를 열 단위로 저장하는 방식 (전통적인 방식: 행 단위로 저장)

장점: 칼럼 단위로 데이터를 압축 → 필요한 칼럼 단위로 로드 → 디스크 I/O 줄임 → 속도 향상

분석에 장점(열 단위 연산 간편 ex. 평균 키)

데이터 마트란?

- 데이터 웨어하우스에서 분석에 필요한 데이터만 추출하여 구성

### 데이터 마트를 비정규화 테이블로 구성하는 것이 가장 단순하고 효율적인 방식이라고 했는데, 그 이유에 대해 함께 논의

1. 정규화된 구조는

- 여러 테이블이 복잡하게 연결 & 중복없이 PK-FK 관계를 따라 쿼리를 짜야함

비정규화 구조는

- 한 테이블 또는 단순한 스타 스키마만 보면 분석 가능
- 고객이름, 지역, 제품명, 시간정보 등이 한 줄(row)에 함께 있음

→ 분석가 입장에서는 비정규화된 구조가 보기에 간편하고 쉬움

1. 비정규화하면 조인 횟수 감소 → 쿼리 속도 빨라짐

따라서 I/O를 줄일 수 있음

1. 데이터 파이프라인이 단순해짐

비정규화 구조는 한번에 로드가 가능하여 ETL이 단순해짐

따라서 데이터 엔지니어가 비정규화된 테이블로 데이터 마트를 구축해놓으면, 분석가는 보다 쉽게 분석이 가능함

그렇다면 항상 비정규화된 테이블로 데이터 마트를 구축하는게 바람직할까?

데이터의 무결성이 중요할 때, 

중복된 데이터가 지나치게 커서 → 스토리지 낭비가 심할 때

**ELT 구조**(Extract → Load → Transform)에서 비정규화는 언제 수행되는가?

1. 데이터 마트는 비정규화 테이블로 만드는게 효율적이고, 데이터의 양이 늘어나면 열지향스토리지를 사용해야한다고 하는데, 그 이유는?

1. 이유: 비정규화된 테이블은 열이 많고(10~100개), 쿼리는 일부 열만 사용하기 때문에 칼럼 단위로 압축하여 접근이 쉬운 열지향스토리지를 사용하는 것이 효율적

예를 들어 분석에 필요한 칼럼이 3개일 때, 열지향 스토리지는 필요한 열만 뽑아올 수 있지만, RDB는 전체 행을 다 불러옴
